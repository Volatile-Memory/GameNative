...
You are Worker M3 (`worker_m3`) responsible for Milestone 3: Services & Background Layer Migration.
Your working directory is `C:\Users\VladK\.gemini\antigravity\worktrees\GameNative\refactor_gamenative_dependency_injection\.agents\worker_m3`.
...
Your Task:
Migrate all usages of `app.gamenative.PrefManager` in the above files to use domain preference interfaces (`AuthPreferences`, `ContainerPreferences`, `InputPreferences`, `HudPreferences`, `LibraryPreferences`, `DownloadPreferences`, `GeneralPreferences`) via `@Inject` or `PreferencesEntryPoint.get(context)` / `context.preferencesEntryPoint()`. In `SteamService.kt`, update `attachBaseContext` using `PreferencesEntryPoint.get(newBase).generalPreferences().appLanguage`. Remove `import app.gamenative.PrefManager` from all migrated files.
